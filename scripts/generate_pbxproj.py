#!/usr/bin/env python3
"""
Generate a valid MusicXNA.xcodeproj/project.pbxproj file with all Swift sources,
...
class PBXProjGenerator:
    ...
    def generate_pbxproj(self):
        ...
        main_target_uuid = generate_uuid("TARGET_MusicXNA")
        product_ref_uuid = generate_uuid("PRODUCT_MusicXNA")
        product_build_uuid = generate_uuid("BUILD_PRODUCT_MusicXNA")
        native_target_uuid = generate_uuid("NATIVETARGET_MusicXNA")
        
        content += f"\n\t/* Products */\n"
        content += f"\t\t{product_ref_uuid} /* MusicXNA.app */ = {{isa = PBXFileReference; explicitFileType = wrapper.application; includeInIndex = 0; path = MusicXNA.app; sourceTree = BUILT_PRODUCTS_DIR; }};\n"
        ...
        # 8. Native target
        content += f"\n\t/* PBXNativeTarget */\n"
        content += f"\t\t{native_target_uuid} /* MusicXNA */ = {{\n"
        ...
        content += f"\t\t\t);\n"
        content += f"\t\t\tname = MusicXNA;\n"
        content += f"\t\t\tpackageProductDependencies = (\n"
        content += f"\t\t\t);\n"
        content += f"\t\t\tproductName = MusicXNA;\n"
        content += f"\t\t\tproductReference = {product_ref_uuid} /* MusicXNA.app */;\n"
        content += f"\t\t\tproductType = \"com.apple.product-type.application\";\n"
        content += f"\t\t}};\n"
        
        # 9. Project
        project_uuid = generate_uuid("PROJECT_MusicXNA")
        content += f"\n\t/* PBXProject */\n"
        ...
        # Main group
        content += f"\t\t{generate_uuid('GROUP_Main')} /* MusicXNA */ = {{\n"
        ...
        content += f"\t\t\tisa = PBXGroup;\n"
        content += f"\t\t\tchildren = (\n"
        content += f"\t\t\t\t{product_ref_uuid} /* MusicXNA.app */,\n"
        content += f"\t\t\t);\n"
        ...
        for conf_name, conf_uuid in [('Debug', generate_uuid('CONF_TargetDebug')), ('Release', generate_uuid('CONF_TargetRelease'))]:
            ...
            content += f"\t\t\t\tCOPYRIGHT = \"Copyright © 2026 MusicXNA. All rights reserved.\";\n"
            content += f"\t\t\t\tCURRENT_PROJECT_VERSION = 1;\n"
            content += f"\t\t\t\tDEVELOPMENT_TEAM = \"\";\n"
            content += f"\t\t\t\tEXECUTABLE_NAME = MusicXNA;\n"
            content += f"\t\t\t\tINFOPLIST_FILE = \"Runner/Info.plist\";\n"
            content += f"\t\t\t\tINFOPLIST_KEY_CFBundleDisplayName = MusicXNA;\n"
            ...
            content += f"\t\t\t\tPRODUCT_NAME = MusicXNA;\n"
...
def main():
    ...
    print(f"📁 Scanning MusicXNA project at: {project_root}")
    
    generator = PBXProjGenerator(project_root)
    generator.scan_files()
    
    print(f"✅ Found {len(generator.source_files)} Swift source files")
    print(f"✅ Found {len(generator.resource_files)} resource files")
