#!/usr/bin/env python3
"""
Validate that MusicXNA.xcodeproj/project.pbxproj includes all required files.
This script checks:
...
def main():
    ...
    pbxproj_file = project_root / 'MusicXNA.xcodeproj' / 'project.pbxproj'
    runner_dir = project_root / 'Runner'
    
    print("🔍 Validating MusicXNA.xcodeproj\n")
    ...
    if missing_swift:
        print(f"\n❌ Missing Swift files ({len(missing_swift)}):")
        for f in sorted(missing_swift):
            print(f"   - {f}")
            errors.append(f"Swift file missing: {f}")
    else:
        print(f"\n✅ All Swift files are in pbxproj")
    
    if missing_resources:
        print(f"\n❌ Missing resource files ({len(missing_resources)}):")
        for f in sorted(missing_resources):
            print(f"   - {f}")
            errors.append(f"Resource missing: {f}")
    else:
        print(f"\n✅ All resource files are in pbxproj")
    ...
    if len(ml_models) >= 4:
        print(f"   ✅ Found {len(ml_models)} CoreML models:")
        for model in sorted(ml_models):
            print(f"      - {model}")
    else:
        print(f"   ⚠️  Only {len(ml_models)} CoreML models found (expected 4+)")
    ...
    if len(sources_uuids) == len(swift_files):
        print(f"   ✅ All Swift files are in Compile Sources")
    else:
        print(f"   ❌ Swift file count mismatch: {len(sources_uuids)} in phase vs {len(swift_files)} on disk")
        errors.append(f"Compile Sources count mismatch")
    
    # Summary
    print(f"\n{'='*60}")
    if not errors:
        print("✅ Xcode project validation PASSED")
